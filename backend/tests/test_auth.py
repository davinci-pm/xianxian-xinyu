...
def test_non_ascii_invite_code_can_login(
    client: TestClient, monkeypatch: MonkeyPatch
) -> None:
    monkeypatch.setattr(
        "app.services.auth.configured_invite_codes", lambda: ["先贤-内测-001"]
    )

    response = client.post("/api/v1/auth/login", json={"invite_code": "先贤-内测-001"})
