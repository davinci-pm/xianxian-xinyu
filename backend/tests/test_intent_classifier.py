...
async def test_clear_direct_request_uses_zero_network_fast_path(monkeypatch: Any) -> None:
    settings = SimpleNamespace(
        intent_llm_enabled=True,
        intent_local_fast_path_enabled=True,
        intent_local_fast_path_threshold=0.82,
    ...
    async def unexpected_remote(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("clear direct request should not call the remote intent model")

    monkeypatch.setattr(
        "app.services.intent_classifier.OpenAICompatibleIntentClassifier.analyze",
        unexpected_remote,
    )

    result = await analyze_intent("你为什么进入加密行业？为什么后来做 TRON？", [])
...
async def test_tell_me_request_uses_zero_network_fast_path(monkeypatch: Any) -> None:
    settings = SimpleNamespace(
        intent_llm_enabled=True,
        intent_local_fast_path_enabled=True,
        intent_local_fast_path_threshold=0.82,
    ...
    async def unexpected_remote(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("explicit tell-me request should not call the remote intent model")
    ...
    result = await analyze_intent("告诉我一件资料没有公开的私人经历。", [])
