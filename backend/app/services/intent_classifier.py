...
class HeuristicIntentClassifier:
    async def analyze(self, user_text: str, recent_messages: list[dict[str, str]]) -> IntentResult:
        ...
        if stage != "END" and (
            len(text) >= 48 or (direct_request and (has_specific_context or len(text) >= 12))
        ):
            stage = "GUIDANCE"
            move = (
                "reflect"
            ...
            should_ask = False
            signal_count = max(signal_count, 2)
...
async def analyze_intent(user_text: str, recent_messages: list[dict[str, str]]) -> IntentResult:
    ...
    if (
        ...
        and (
            fallback_result.analysis.primary_intent == "end"
            or fallback_result.analysis.recommended_stage == "GUIDANCE"
        )
    ):
        return fallback_result
    started = monotonic()
