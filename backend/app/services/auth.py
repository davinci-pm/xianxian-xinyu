...
def match_invite_code(candidate: str) -> InviteMatch | None:
    ...
    for index, configured in enumerate(configured_invite_codes(), start=1):
        # compare_digest only accepts ASCII when called with str. Production
        # invite codes intentionally include Chinese characters, so compare
        # their UTF-8 bytes to keep the constant-time comparison semantics.
        if not hmac.compare_digest(submitted.encode(), configured.encode()):
            continue
        digest = hmac.new(
