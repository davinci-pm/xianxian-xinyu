...
> 数据方案：SQLite `/tmp` 运行副本＋TOS 压缩去重备份＋启动恢复
...
- SQLite 活跃文件放在 `/tmp`；
- TOS 只在数据库发生变化时保存单份压缩快照，并保留最近 3 个版本；
- 后端最小实例为 0、最大实例为 1，依靠启动恢复和优雅关停备份兼顾成本与数据持久化。
...
1. 获取后端 HTTPS 地址；
2. 调用 `/api/v1/health`；
3. 设置后端 `--minInstance 0 --maxInstance 1`，避免空闲常驻费用和 SQLite 多实例写入；
4. 测试创建会话和真实流式消息；
5. 测试 TOS 备份。
...
- 前后端最小实例均保持为 0，后端最大实例保持为 1；
- 数据库备份默认每天巡检一次，并在启动/优雅关停时执行；内容哈希未变化则不上传；
- TOS 快照使用 gzip 压缩且每轮只上传一份，禁止通过公网每几分钟重复上传完整数据库；
- 限制单条输入和最大输出 Token；
- 单会话设置最大轮数软提醒；
- 每个邀请码设置日调用上限；
- 创建预算告警；
