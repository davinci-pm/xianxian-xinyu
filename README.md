...
- Markdown 资料按标题与语义切分，使用按人物惰性加载的 LRU 内存索引、BM25 + 向量检索 + RRF 融合排序。
- 当前语料规模为 255 份资料、约 10,000 个知识分块；每轮只注入最相关的少量片段。
- 对话阶段管理、主动追问、快捷回复、停止生成与失败降级；慢响应提供持续状态，断流可幂等重试。
- 普通对话使用快速生成，只在复杂权衡时自适应启用低强度思考；长 Skill 只注入本轮相关的人物方法。
- 会话历史、语义识别的重要长期记忆，以及可编辑的心语札记。
- 心语札记可导出 Markdown、Word 或 PDF；完整对话可按需作为附录导出。
- 只有明确且紧迫的本人现实危险会在模型调用前中断人物表达；普通痛苦、否定、引用和文学讨论保持人物对话。
- 一人一码的邀请制登录，可在不同设备恢复同一账号的数据。
- 女娲 Soul V3：导入微信聊天、文章、演讲等资料，按数据质量自适应生成证据、决策与表达模型。
- 真实留出集与 LoRA 训练门槛；留出证据不足时不会用结构分冒充高保真，也不会盲目微调。
- 在世人物近期问题使用独立联网事实层，公开事件、稳定人格与推断严格分层并返回可点击来源。
...
“女娲”已经作为邀请制内测功能上线。用户可以上传自己有权使用的聊天记录、文章、演讲或时间线；系统会先判断资料结构和质量，再启用当前证据真正支持的能力。
...
- [x] V1.1：流式心跳、断流检测、安全重试与对话暂停误判修复
- [x] V1.1：最小安全边界、语境识别和非紧迫敏感内容完整输出
- [x] V1.1：检索索引惰性缓存、Skill 相关片段路由与模型语义自适应生成
- [ ] V1.2：进一步增强人物间的语言、价值排序与推理差异
- [ ] V1.2：人物资料版本、增量更新与来源审核后台
- [x] V2：女娲资料上传、质量体检、Soul V3 蒸馏、预览和人物创建工作流
- [x] V2：在世人物按需联网事实层、来源引用与人格事实分层
- [ ] V2.1：在世人物公开资料的定期后台更新与版本对比
...
# 启动真实模型后的内容价值回归（身份、人物方法、行动性、追问数）
.venv/bin/python -m app.scripts.benchmark_dialogue_quality --base-url http://127.0.0.1:8765
...
Playwright 会在桌面和移动视口验证人物发现、对话、记忆、札记、安全恢复、日常负面表达和断流重试主流程。
...
- 只有本人、当前或近期、明确行动同时成立的紧迫危险进入平台安全流程；敏感词本身不构成暂停条件。
